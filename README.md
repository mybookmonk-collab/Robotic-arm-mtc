...
假设你的工作空间是~/elfin_robot，你需要运行的命令如下：
```sh
$ cd ~/elfin_robot/src
$ colcon build --packages-select gripper_hardware_interface --symlink-install --cmake-clean-cache
$ colcon build --packages-select crt_ctag2f90c_gripper_visualization --symlink-install --cmake-clean-cache
$ colcon build --base-paths src/elfin_robot_ros2-humble_ethercat/
$ colcon build --packages-select trac_ik_lib --symlink-install --cmake-clean-cache
$ colcon build --packages-select trac_ik_examples --symlink-install --cmake-clean-cache
$ colcon build --packages-select trac_ik_kinematics --symlink-install --cmake-clean-cache
$ colcon build --packages-select trac_ik --symlink-install --cmake-clean-cache
$ colcon build --packages-select mtc_tutorial --symlink-install --cmake-clean-cache
```
---
...
用Gazebo仿真和Moveit请运行：
```sh
$ ros2 launch elfin3_ros2_moveit2 pelfin3.launch.py
```
...
```
![](assets/17605088861966.jpg)

执行mtc运动规划：
```sh
$ ros2 launch elfin3_ros2_moveit2 mtc_demo.launch.py
```
